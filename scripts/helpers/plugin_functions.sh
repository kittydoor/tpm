...
_tpm_path() {
	local string_path
	string_path="$(tmux start-server\; show-environment -g TMUX_PLUGIN_MANAGER_PATH | cut -f2 -d=)/"
...
# 2. "user/plugin_name"
plugin_name_helper() {
	local plugin plugin_basename plugin_name
	...
	plugin_name="${plugin_basename%.git}"
	echo "$plugin_name"
}

plugin_path_helper() {
	local plugin="$1" plugin_name
	plugin_name="$(plugin_name_helper "$plugin")"
	echo "$(tpm_path)${plugin_name}/"
}

plugin_already_installed() {
	local plugin="$1" plugin_path
